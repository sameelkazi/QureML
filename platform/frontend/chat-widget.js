    ...
    for (const url of candidateEndpoints) {
      try {
        ...
        if (res.ok) {
          ...
          throw new Error(errData.detail || errData.error || `Server responded with HTTP ${res.status}`);
        }
      } catch (err) {
        lastResponse = err;
        // Continue to try next candidate endpoint if it was a network failure
      }
    }
